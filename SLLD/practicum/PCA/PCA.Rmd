...
The function \textbf{princomp()} in the \textbf{stats} package uses the spectral decomposition, \textbf{prcomp()} which is part of the package \textbf{FactoMineR} uses the singular value decomposition (SVD).
...
Thus, it is common fix a priori an \textbf{acceptance threshold} (e.g. 80%) and look at the \textbf{cumulative PVE}.
